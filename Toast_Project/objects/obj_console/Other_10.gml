					...
					if (!object_exists(object_to_try_and_edit)) { // object doesnt exist 
						...
						if (instance_number(object_to_try_and_edit) == 0) { // object doesnt exist 
							...
							with (object_to_try_and_edit) { // many things might have this 
								...
								} else if (is_array(getting_this_instance)) { // ARRAY OF STRINGS
									var new_array = array_create(array_length_1d(getting_this_instance),string(other.text_part[3]))
									variable_instance_set(id, string(other.text_part[2]), new_array)
								} else { // STRING
									variable_instance_set(id, string(other.text_part[2]), string(other.text_part[3]))
								}
	...
	#region game
		...
		if (parse_parts == 1) { // object doesnt exist 
			...
			if (text_part[1] == "restart") {
				...
				#endregion
			} else if (text_part[1] == "debug_overlay_on") {
				#region debug_overlay_on
				show_debug_overlay(true)
				command_done = true
				#endregion
			} else if (text_part[1] == "debug_overlay_off") {
				#region debug_overlay_off
